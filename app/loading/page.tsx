...
export default function LoadingPage() {
  ...
  return (
    <div className="gradient-bg min-h-screen flex flex-col items-center justify-center p-6">
      ...
      <h1 className="text-white text-3xl font-bold mb-2 text-center">Analyzing Your Quote</h1>
      <p className="text-white/80 text-center mb-12 px-4">Our AI is working its magic...</p>
      ...
      <div className="w-full max-w-md space-y-6 mb-12">
        {steps.map((step, index) => (
          ...
          >
            <div className={`bg-white rounded-2xl p-5 shadow-xl ${index === currentStep ? 'ring-4 ring-white/30' : ''}`}>
              <div className="flex items-center gap-4">
                <div className={`w-14 h-14 bg-gradient-to-br ${step.color} rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0 ${index === currentStep ? 'animate-pulse' : ''}`}>
                  <i className={`fas ${step.icon} text-white text-2xl`}></i>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-gray-900 text-lg">{step.title}</h3>
                    {index < currentStep && <i className="fas fa-check-circle text-green-500 text-lg"></i>}
                    {index === currentStep && (
        ...
        <div className="flex items-start gap-3 text-white">
          ...
          <div>
            ...
            <p className="text-sm text-white/80">
              Most car owners overpay 20–40% on repairs due to lack of price transparency.
            </p>
          </div>
        </div>
      </div>
